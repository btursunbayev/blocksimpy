    ...
    def check_range(self, value, expected, tolerance, description):
        ...
        if not in_range:
            self.check(False, description, 
                      f"{value:.2f}", f"{expected:.2f} ±{tolerance*100:.0f}%")
        else:
...
def test_bitcoin():
    ...
    config['transactions']['transactions_per_wallet'] = STANDARD_TEST_CONFIG['transactions_per_wallet']
    ...
    test.check_range(results['avg_block_time'], target_blocktime, STANDARD_TEST_CONFIG['tolerance'],
                    f"Average block time within ±{STANDARD_TEST_CONFIG['tolerance']*100:.0f}%")
    test.check(results['total_coins'] == blocks * initial_reward,
    ...
    if results['tps'] > expected_tps_max:
        test.warn(f"TPS ({results['tps']:.2f}) exceeds theoretical max ({expected_tps_max:.2f})")
...
def test_litecoin():
    ...
    test.check(results['total_coins'] == blocks * initial_reward,
              "Coin issuance is correct")
    test.check(results['avg_block_time'] < 200,
              "Block time faster than Bitcoin (as expected)")
...
def test_dogecoin():
    ...
    if config['economics']['max_halvings'] == 0:
        test.check(results['total_coins'] == 0,
                  "No coins issued (Dogecoin configured with no halvings)")
    else:
        test.check(results['total_coins'] == blocks * initial_reward,
                  "Coin issuance is correct (high reward)")
        test.check(results['total_coins'] > 100000,
                  "High coin supply (Dogecoin characteristic)")
    
    test.check(results['avg_block_time'] < 100,
              "Block time much faster than Bitcoin")
...
def test_bitcoin_cash():
    ...
    test.check(results['total_tx'] >= results['blocks'],
...
def test_halving():
    ...
    test.check(abs(results['total_coins'] - expected_coins) < 0.01,
              "Halving mechanism works correctly",
              results['total_coins'], expected_coins)
    
    # Check that coins issued is less than if no halving
    no_halving_coins = 25 * 50.0
    test.check(results['total_coins'] < no_halving_coins,
              "Halving reduces coin issuance",
              results['total_coins'], f"< {no_halving_coins}")
...
def test_network_metrics():
    ...
    test.check(results['io_requests'] > 0,
              "I/O requests tracked")
    test.check(results['network_data'] > 0,
              "Network data tracked")
    ...
    expected_io = results['blocks'] * config['network']['nodes'] * config['network']['neighbors']
    test.check(results['io_requests'] == expected_io,
    ...
    bytes_per_io = results['network_data'] / results['io_requests'] if results['io_requests'] > 0 else 0
    test.check(bytes_per_io > 1000 and bytes_per_io < 100000,
