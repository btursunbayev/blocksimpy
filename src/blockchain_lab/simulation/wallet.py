...
def wallet(env: simpy.Environment, wallet_id: int, transaction_count: int, 
          interval: float, transaction_pool: List[Tuple[int, float]]):
    """
...
def create_wallets(env: simpy.Environment, wallet_count: int, transactions_per_wallet: int,
                   interval: float, transaction_pool: List[Tuple[int, float]]) -> List[simpy.Process]:
    """
    ...
    for wallet_id in range(wallet_count):
        process = env.process(wallet(env, wallet_id, transactions_per_wallet, 
                                   interval, transaction_pool))
