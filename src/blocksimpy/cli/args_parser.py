...
Examples:
  ...
  %(prog)s --nodes 50 --neighbors 8 --miners 20 --blocks 100
    ...
    parser.add_argument("--chain", type=str, metavar="TYPE", default="defaults",
    ...
    network_group.add_argument("--nodes", type=int, metavar="N",
                              help="Number of peer nodes, each maintaining stored block IDs")
    network_group.add_argument("--neighbors", type=int, metavar="M", 
    ...
    mining_group.add_argument("--miners", type=int, metavar="K",
                             help="Number of mining processes competing for blocks")
    mining_group.add_argument("--hashrate", type=float, metavar="H",
                             help="Hashrate per miner (total_hashrate affects block timing)")
    mining_group.add_argument("--difficulty", type=float, metavar="D",
                             help="Mining difficulty (auto-calculated as blocktime × total_hashrate if not set)")
    mining_group.add_argument("--blocktime", type=float, metavar="T",
                             help="Target block time in seconds (used for difficulty retargeting)")
    mining_group.add_argument("--blocksize", type=int, metavar="B",
    ...
    economics_group.add_argument("--reward", dest="init_reward", type=float, default=50, metavar="R",
                                help="Initial coinbase reward per block (default: 50)")
    economics_group.add_argument("--halving", dest="halving_interval", type=int, default=210000, metavar="H",
    ...
    tx_group.add_argument("--wallets", type=int, metavar="W",
                         help="Number of wallet processes generating transactions")
    tx_group.add_argument("--transactions", type=int, metavar="X", 
                         help="Number of transactions each wallet will generate")
    tx_group.add_argument("--interval", type=float, metavar="I",
    ...
    control_group.add_argument("--blocks", type=int, metavar="L",
                              help="Maximum blocks to mine (simulation ends when reached or all txs confirmed)")
    control_group.add_argument("--years", type=float, metavar="Y",
    ...
    output_group.add_argument("--print", dest="print_int", type=int, default=144, metavar="P",
